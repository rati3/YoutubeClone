$(function(){
    ...
    initCommentForm($createCommentForm,
        () => {
            resetForm();
        }, () => {
            $.ajax({
    ...
    function initCommentForm($form,
        ...
        if (placeholder) $textarea.attr('placeholder', placeholder);
        if (btnSAveText) $save.text(btnSAveText);
        ...
        $form.submit(ev => {
            ev.preventDefault();
            if (submitCb && typeof submitCb === 'function') submitCb();
    ...
    function initComment($comment) {
        ...
        function onCommentPin(ev) {
            ...
            if (pinned) {
                ...
                if(!confirm("You are about to pin the comment. Are you sure you want to unpin any other comments?")) return;
            }
        ...
        function onReplyClick(ev) {
            ...
            initCommentForm(
                ...
                () => {
                    ...
                    if (value.indexOf('@'+mentionUsername) !== 0) mentionUsername = null;
        ...
        function loadSubComments(ev) {
            ...
            if (commentsCollapsed) $subCommentSection.css('display', 'none');
            else $subCommentSection.css('display', 'block');
