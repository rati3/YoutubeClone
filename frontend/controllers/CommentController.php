...
class CommentController extends Controller
{
    public function behaviors()
    {
        return [
            ...
            'content' => [
                ...
                'only' => ['create', 'delete', 'update', 'reply', 'by-parent', 'pin'],
                'formats' => [
    ...
    public function actionDelete($id)
    {
        ...
        if ($comment->belongsTo(\Yii::$app->user->id) ||
            $comment->video->belongsTo(\Yii::$app->user->id)) {
            $comment->delete();
            return ['success' => true];
        }
    ...
    public function actionReply()
    {
        ...
        if (strpos($commentText, '@'.$mentionUsername) !== 0) $mentionUsername = null;
        else $commentText = trim(str_replace('@'.$mentionUsername.'', '', $commentText));


        if ($mentionUsername) {
            $mentionUser = User::findByUsername($mentionUsername);
            if (!$mentionUser) $mentionUsername = null;
            else {
        ...
        if ($parentComment->parent_id) $comment->parent_id = @$parentComment->parent_id;
        else $comment->parent_id = $parentId;
